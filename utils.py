...
def  qr_decomposition(matrix):
    A_np = matrix.cpu().detach().numpy()
    Q_np, R_np, pivots = qr(A_np, pivoting=True)
    Q = torch.tensor(Q_np, device=device)
    R = torch.tensor(R_np, device=device)
    P = torch.tensor(pivots, device=device)
    return Q, R, P
...
def low_rank_qr_decomposition(weight, rank_ratio=0.1, parameter_ratio=0.15,
                           log_level='INFO',
                           return_dict=False):
    """
    ...
    # Use QR to decompose a matrix
    QQ, RR, PP = qr_decomposition(weight)
    PP = PTM(PP)
    QQ = torch.tensor(QQ, device=device)
    RR = torch.tensor(RR, device=device)
    PP = torch.tensor(PP, device=device).float()
    ...
    Rmod = RR[0:reduced_rank, :]
    rank = torch.linalg.matrix_rank(Rmod)
    is_full_rank = rank == min(H, W)

    L = QQ[:, 0:reduced_rank]
    R = Rmod @ torch.linalg.inv(PP)
    ...
    if log_level == 'DEBUG':
        print(f"W: ({H},{W}) | Rank: {rank} | QQ:{QQ.shape} | RR:{RR.shape} | PP:{PP.shape}")
        print(f"Reduced Rank: {reduced_rank} | Num Parameters: {(H + W) * reduced_rank}")
        print(f"L: {L.shape} | R: {R.shape}")

    if return_dict:
        return {"L": L, "R": R, 'reduced_rank': reduced_rank}
    else:
        return L, R
...
def row_avg(tensor):
    ravg = torch.mean(tensor, dim=1)
    diagonal_matrix = torch.diag(ravg)

    return diagonal_matrix

def apply_row_avg(dict):
    modified_dict = {}
    for key, value in dict.items():
        if len(value.shape) == 2:
            modified_value = row_avg(value)
            modified_dict[key] = modified_value
        else:
            modified_dict[key] = value
    return modified_dict

def substitute_layer_weights(module, module2,
                             allow_name=None,
                             block_name=None,
    ...
    for attr_str in dir(module):
        ...
        if do_fwsvd:
            try:
                target_attr2 = getattr(module2, attr_str)
            except:
                pass
        ...
        if type(target_attr) == nn.Linear: # and any(attr_str in an for an in allow_name):
            ...
            if do_svd:
                ...
                if do_fwsvd:
                    if len(target_attr.weight.shape) == 2:
                        WW = nn.Parameter(torch.mm(row_avg(target_attr2.weight), target_attr.weight))
                else:
                    WW = target_attr.weight
                print(type(target_attr.weight), type(WW))
                output = low_rank_decomposition(target_attr.weight, parameter_ratio=parameter_ratio,
                                                return_dict=True, **kwargs)
                L, R, reduced_rank = output['L'], output['R'], output['reduced_rank']
                if do_fwsvd:
                    S = torch.mm(torch.linalg.inv(row_avg(target_attr2.weight)), WW - torch.mm(L, R))
                else:
                    S = WW - torch.mm(L, R)
            ...
            if do_qr:
                # Decompose a matrix by QR
                output = low_rank_qr_decomposition(target_attr.weight, parameter_ratio=parameter_ratio,
                                                return_dict=True, **kwargs)
                L, R, reduced_rank = output['L'], output['R'], output['reduced_rank']
    ...
    for name, immediate_child_module in module.named_children():
        ...
        if not any(name in bn for bn in block_name):
            try:
                corr_module = getattr(module2, name)
                substitute_layer_weights(immediate_child_module, corr_module, allow_name, block_name, parameter_ratio,
                                     has_sparse, do_svd, do_fwsvd, do_qr, fwsvdfile, **kwargs)
            except:
                pass
